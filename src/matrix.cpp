...
const Matrix Matrix::identity(2, 2, ComplexVect({1, 0, 0, 1}));
...
const Matrix qb::cross(const Matrix &a, const Matrix &b)
{
    ...
    const size_t nm = n * m;
    ComplexVect cells;
    cells.insert(cells.begin(), nm * nm, 0);
    for (size_t i = 0; i < n; i++)
    {
